    ...
    if (isset($_POST['login'], $_POST['password_connexion'])) {
        ...
        try {
            ...
            if ($user && password_verify($password_connexion, $user['password'])) {
                ...
                if($premiere_co == "0"){
                    header("Location: changement_mdp.php?login=" . urlencode($user['login']) . "&role=" . urlencode($user['role']));
                    exit();

                }
                header("Location: index.php?login=" . urlencode($user['login']) . urlencode($user['role']));
                exit();
            } else {
